...
func TestCountWordFrequency(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("Test Case #%d", i+1), func(subT *testing.T) {
			result := wordhelper.CountWordFrequency(tc.input)

			if !reflect.DeepEqual(result, tc.expected) {
				subT.Errorf("Expected %v but got %v", formatMap(tc.expected), formatMap(result))
			}
		})
	}
}
